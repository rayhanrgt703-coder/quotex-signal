...
# Strategy params (tweakable)
ATR_PERIOD = 14
ADX_PERIOD = 14
ADX_THRESHOLD = 20
MIN_ATR_THRESHOLD = {
    "EUR_USD": 0.00005, "GBP_USD": 0.00010, "USD_JPY": 0.01,
    "USD_CAD": 0.00008, "AUD_USD": 0.00008, "USD_CHF": 0.00008
}
...
class Indicators:
    def __init__(self):
        self.prices = deque(maxlen=5000)
        self.highs = deque(maxlen=5000)
        self.lows = deque(maxlen=5000)
        self.tr = deque(maxlen=5000)
        self.plus_dm = deque(maxlen=5000)
        self.minus_dm = deque(maxlen=5000)
        self.atr = None
        self.dx = deque(maxlen=5000)
        self.adx = None
        self.ema_fast = None
        self.ema_slow = None
        self.macd_signal = None
        self.macd_hist_list = deque(maxlen=5000)

    def update_with_candle(self, o,h,l,c):
        self.prices.append(c); self.highs.append(h); self.lows.append(l)
...
class TF_Agg:
    def __init__(self):
        ...
        self.m1_history = deque(maxlen=2000)
        self.m5_history = deque(maxlen=1000)

    def add_tick(self, price, time_dt: datetime):
    ...
    def get_structure(self):
        if len(self.m1_history) < 6: return None
        last6 = list(self.m1_history)[-6:]
        highs = [c['high'] for c in last6]; lows = [c['low'] for c in last6]
        if highs[-1] > highs[-3] and lows[-1] > lows[-3]: return 'HH_HL'
