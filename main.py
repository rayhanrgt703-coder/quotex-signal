"""
FINAL PREMIUM main.py (100% FIXED)
----------------------------------
✔ Auto Start OANDA Stream (No /start needed)
✔ Tick-by-tick real-time stream
✔ M1 & M5 candle system
✔ Indicators: MA, RSI, MACD, Bollinger, ATR, ADX
✔ Candlestick pattern engine
✔ Market structure analyzer
✔ Signal generator (85–95%)
...
class Indicators:
    ...
    def _update_adx(self):
        if len(self.plus_dm) >= ADX_PERIOD:
...
class TF_Agg:
    ...
    def finalize_m1(self, candle):
        ...
        ind_m1 = self.ind_m1.update_with_candle(
...
aggs = {s: TF_Agg() for s in INSTRUMENTS.split(",")}
...
def signal_from_indicators(symbol, candle, ind_m1, structure, prev_m1):
    ...
    m1_side = None
    if price > ma and rsi > 55 and macd["hist"] > 0:
        m1_side = "BUY"
    elif price < ma and rsi < 45 and macd["hist"] < 0:
        m1_side = "SELL"
    else:
        return None
    ...
    if pattern in ["HAMMER", "BULLISH_PINBAR", "BULLISH_ENGULFING"]:
        pattern_bias = "BUY"
    if pattern in ["SHOOTING_STAR", "BEARISH_PINBAR", "BEARISH_ENGULFING"]:
        pattern_bias = "SELL"
    if pattern == "DOJI":
        return None
...
def home():
    return HTMLResponse("""
    <h2>Realtime OANDA Premium Stream</h2>
    <p>WS: <b>/ws</b></p>
    <p>Signals: <b>/signals</b></p>
    """)
